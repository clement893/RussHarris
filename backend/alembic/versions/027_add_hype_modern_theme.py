"""Add Hype Modern Theme (ID 34)

Revision ID: 027_add_hype_modern_theme
Revises: 026_add_luxury_theme
...
revision = '027_add_hype_modern_theme'
down_revision = '026_add_luxury_theme'
branch_labels = None
depends_on = None


def upgrade():
    """Create Hype Modern Theme (ID 34) with complex modern hype design."""
    conn = op.get_bind()
    ...
    try:
        ...
        if 'themes' not in tables:
            print("⚠️  Themes table does not exist, skipping Hype Modern Theme creation")
            return
        
        # Check if Hype Modern Theme (ID 34) already exists
        result = conn.execute(text("SELECT id, is_active FROM themes WHERE id = 34"))
        existing_theme = result.fetchone()
        
        if existing_theme:
            print("✅ Hype Modern Theme (ID 34) already exists")
            return
        
        # Hype Modern Theme Configuration
        # Features: Neon colors, advanced glassmorphism, complex gradients, glow effects, animations
        hype_config = {
            "mode": "system",
            "primary_color": "#00F5FF",  # Neon cyan
            "secondary_color": "#FF00F5",  # Neon magenta
            "danger_color": "#FF006E",  # Neon pink
            "warning_color": "#FFB800",  # Neon orange
            "info_color": "#00D9FF",  # Light cyan
        ...
        config_json = json.dumps(hype_config)
        
        # Insert Hype Modern Theme with ID 34 (not active by default)
        conn.execute(text("""
            INSERT INTO themes (id, name, display_name, description, config, is_active, created_by, created_at, updated_at)
            VALUES (34, 'HypeModernTheme', 'Hype Modern Theme', 
        ...
        print("✅ Created Hype Modern Theme (ID 34)")
        print("   - Neon color palette (cyan #00F5FF, magenta #FF00F5, orange #FFB800)")
        print("   - Advanced glassmorphism with heavy blur (20px)")
        print("   - Multiple gradient types (linear, radial, neon)")
        print("   - Neon glow shadows and effects")
        print("   - Complex animations (pulse, glow, shimmer, float)")
        print("   - Modern dark background (#0A0A0F)")
        print("   - Hype component styles with neon borders")
        print("   - Extended typography scale (xs to 6xl)")
        print("   - Advanced spacing system (xs to 5xl)")
        
    except Exception as e:
        print(f"❌ Error creating Hype Modern Theme: {e}")
        raise


def downgrade():
    """Remove Hype Modern Theme (ID 34) if it exists."""
    conn = op.get_bind()
    ...
    if 'themes' not in tables:
        print("⚠️  Themes table does not exist, skipping Hype Modern Theme removal")
        return
    
    # Check if Hype Modern Theme exists
    result = conn.execute(text("SELECT id, is_active FROM themes WHERE id = 34"))
    existing_theme = result.fetchone()
    ...
    if existing_theme:
        ...
        if existing_theme[1]:  # is_active
            ...
            other_theme_result = conn.execute(text("SELECT id FROM themes WHERE id != 34 AND is_active = false LIMIT 1"))
            other_theme = other_theme_result.fetchone()
            
            if other_theme:
                conn.execute(text("UPDATE themes SET is_active = true WHERE id = :id"), {"id": other_theme[0]})
                print(f"   Activated theme ID {other_theme[0]} before removing Hype Modern Theme")
        
        # Remove Hype Modern Theme
        conn.execute(text("DELETE FROM themes WHERE id = 34"))
        print("✅ Removed Hype Modern Theme (ID 34)")
    else:
        print("ℹ️  Hype Modern Theme (ID 34) does not exist, nothing to remove")
